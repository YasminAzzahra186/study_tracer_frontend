  ...
  return (
    <div className="p-8 md:p-10 flex-1 animate-in fade-in duration-300">
      <div className="flex items-center justify-between mb-8">
        <div>
          <h2 className="text-xl font-black text-[#3C5759] tracking-tight">Detail Pribadi</h2>
          <p className="text-sm text-[#3C5759]/60">Informasi dasar akun Anda.</p>
        </div>
        {isEditing ? (
          <div className="flex items-center gap-2">
            ...
            <button onClick={handleSaveProfile} disabled={saving} className="flex items-center gap-2 px-4 py-2 bg-[#3C5759] text-white rounded-xl text-xs font-bold shadow-md hover:bg-[#2A3E3F] transition-all cursor-pointer disabled:opacity-50">
              {saving ? <Loader2 size={14} className="animate-spin" /> : <Save size={14} />} Simpan
            </button>
          </div>
        ) : (
          <button onClick={startEditing} className="flex items-center gap-2 px-4 py-2 bg-[#3C5759]/10 text-[#3C5759] rounded-xl text-xs font-bold hover:bg-[#3C5759]/20 transition-all cursor-pointer">
