...
const DUMMY_USER = {
  ...
  karier: {
    saatIni: {
      judulJob: "Senior Product Engineer",
      perusahaan: "TechFlow Systems",
      tanggalMulai: "March 2020",
      tanggalSelesai: "January 2023",
      kota: "Surabaya",
      provinsi: "Jawa Timur"
    },
    riwayat: [
      {
        id: 1,
        judulJob: "Junior Developer",
        perusahaan: "Innovate Corp",
          ...
          <div className="lg:col-span-8 bg-white rounded-[2rem] shadow-sm border border-slate-100 overflow-hidden flex flex-col">
            ...
            <div className="p-8 md:p-10 flex-1">
              <div className="flex items-center justify-between mb-6">
                ...
                <button className="text-xs font-bold text-[#3C5759] hover:underline cursor-pointer">
              ...
              <div className="border-2 border-dashed border-slate-200 rounded-[1.5rem] p-6 mb-6 bg-white">
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
                  <div>
                    <label className="block text-[10px] font-black text-[#3C5759]/40 uppercase tracking-widest mb-2">Judul Job</label>
                    <div className="relative">
                      <select disabled className="w-full bg-slate-50 border border-slate-100 rounded-xl px-4 py-3 text-sm font-semibold text-[#3C5759] appearance-none focus:outline-none" defaultValue={DUMMY_USER.karier.saatIni.judulJob}>
                        <option>{DUMMY_USER.karier.saatIni.judulJob}</option>
                      </select>
                      <ChevronDown size={16} className="absolute right-4 top-1/2 -translate-y-1/2 text-[#3C5759]/50 pointer-events-none" />
                    </div>
                  </div>
                  <div>
                    <label className="block text-[10px] font-black text-[#3C5759]/40 uppercase tracking-widest mb-2">Perusahaan</label>
                    <div className="relative">
                      <select disabled className="w-full bg-slate-50 border border-slate-100 rounded-xl px-4 py-3 text-sm font-semibold text-[#3C5759] appearance-none focus:outline-none" defaultValue={DUMMY_USER.karier.saatIni.perusahaan}>
                        <option>{DUMMY_USER.karier.saatIni.perusahaan}</option>
                      </select>
                      <ChevronDown size={16} className="absolute right-4 top-1/2 -translate-y-1/2 text-[#3C5759]/50 pointer-events-none" />
                    </div>
                  </div>
                  <div>
                    <label className="block text-[10px] font-black text-[#3C5759]/40 uppercase tracking-widest mb-2">Tanggal Mulai</label>
                    <input type="text" readOnly value={DUMMY_USER.karier.saatIni.tanggalMulai} className="w-full bg-slate-50 border border-slate-100 rounded-xl px-4 py-3 text-sm font-semibold text-[#3C5759] focus:outline-none" />
                  </div>
                  <div>
                    <label className="block text-[10px] font-black text-[#3C5759]/40 uppercase tracking-widest mb-2">Tanggal Selesai <span className="lowercase normal-case font-medium">(optional)</span></label>
                    <input type="text" readOnly value={DUMMY_USER.karier.saatIni.tanggalSelesai} className="w-full bg-slate-50 border border-slate-100 rounded-xl px-4 py-3 text-sm font-semibold text-[#3C5759] focus:outline-none" />
                  </div>
                  <div>
                    <label className="block text-[10px] font-black text-[#3C5759]/40 uppercase tracking-widest mb-2">Kota</label>
                    <div className="relative">
                      <select disabled className="w-full bg-slate-50 border border-slate-100 rounded-xl px-4 py-3 text-sm font-semibold text-[#3C5759] appearance-none focus:outline-none" defaultValue={DUMMY_USER.karier.saatIni.kota}>
                        <option>{DUMMY_USER.karier.saatIni.kota}</option>
                      </select>
                      <ChevronDown size={16} className="absolute right-4 top-1/2 -translate-y-1/2 text-[#3C5759]/50 pointer-events-none" />
                    </div>
                  </div>
                  <div>
                    ...
                    <div className="relative">
                      <select disabled className="w-full bg-slate-50 border border-slate-100 rounded-xl px-4 py-3 text-sm font-semibold text-[#3C5759] appearance-none focus:outline-none" defaultValue={DUMMY_USER.karier.saatIni.provinsi}>
                        <option>{DUMMY_USER.karier.saatIni.provinsi}</option>
                      </select>
                      <ChevronDown size={16} className="absolute right-4 top-1/2 -translate-y-1/2 text-[#3C5759]/50 pointer-events-none" />
                    </div>
                  </div>
              ...
              <div className="space-y-4">
                {DUMMY_USER.karier.riwayat.map((item) => (
                  <div key={item.id} className="bg-slate-50 border border-slate-100 rounded-[1.5rem] p-6 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                    <div>
                      <h3 className="text-[15px] font-black text-[#3C5759] mb-1">{item.judulJob}</h3>
                      <p className="text-sm font-semibold text-[#3C5759]/60">{item.perusahaan}</p>
                      <p className="text-sm font-semibold text-[#3C5759]/60">{item.lokasi}</p>
                    </div>
